...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the LoRA debate pipeline from a config file."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the LoRA pipeline config JSON file.",
    )
...
def main():
    ...
    if config.get("condition") != "lora":
        raise ValueError(f"Expected condition='lora', got: {config.get('condition')}")
